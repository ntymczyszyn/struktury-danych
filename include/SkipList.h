...
public:
    ...
    int compare_to(std::unique_ptr<Node<T>> node2);
    std::unique_ptr<Node<T>> find(std::unique_ptr<Node<T>> f);
    void insert(std::unique_ptr<Node<T>> node2, std::unique_ptr<Node<T>> lower, int insert_height, int distance);
    void remove(std::unique_ptr<Node<T>> node2);
...
public:
    ...
    ~SkipList();
    void insert(std::unique_ptr<Node<T>> node2);
    void remove(std::unique_ptr<Node<T>> node2);
    int get_rank(std::unique_ptr<Node<T>> node2);
